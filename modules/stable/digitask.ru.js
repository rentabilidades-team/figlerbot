import { load_wait, form_get, insert, inject, base_domain, close_module } from "../core/resources/libglobal.js";
...
async function iniciar_modulo() {
    ...
    if (window.location.pathname.indexOf('/notimer_fp/faucet.php') >= 0) {
        if (form_get('guardarurl') == '1') {
            location.replace('//' + base_domain() + '/modulo-digitask?digitaskurl=' + encodeURIComponent(window.location.href));
        } else {
            close_module();
        }
    }
