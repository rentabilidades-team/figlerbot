...
async function iniciar_modulo() {
  setInterval(function () {
    if ($('div[class^="_"]').length > 0) {
      $('div[class^="_"]').click();
    } //Cierra la publicidad inicial
    if ($("span#spins").html() != 0) {
      if (
        ($("input#playFancy").is(":enabled") == true &&
          $("input#playFancy").is(":visible") == true &&
          $("input#playFancy").val() == "") ||
        $("input#playFancy").val() == "Play"
      ) {
        click("input#playFancy");
      }
      var numaleatorio;
      ...
      if (
        $("div#runModal > div > div > div.modal-header > a > span").is(
          ":visible"
        ) == true
      ) {
