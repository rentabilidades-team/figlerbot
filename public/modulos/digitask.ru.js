...
async function iniciar_modulo() {
    if(window.location.pathname.indexOf('/notimer_fp')>=0){
        if(libglobal.obtener_get('fpdoge')!=null){$('form > div > input[type=text]:nth-child(1)').val(obtenget('fpdoge'));}
        if(libglobal.obtener_get('importar')==1){$('form').append( "<input type='hidden' name='guardarurl' value='1'>");}
    }
